import json
from attentiveClass import ProductClass, PriceClass, ProductOptionValueClass, ProductOptionsClass, VariantClass, ImagesClass, ProductEncoder

products = []
# Opening Jockey JSON file
# returns JSON object as a dictionary
# r = filename of Jockey File
with open('C:\\Users\\vorlobm\\Documents\\Attentive\\bigBoy.json', "r") as r:
    data = json.load(r)
...
for item in data['ItemCollection']['Item']:
    ...
    for variant in item['ns1:VariationCollection']['ns1:VariationData']:
        ...
        for currentprice in variant['ns1:PriceCollection']['ns1:PriceData']:
            if currentprice.get('@priceType') == "ListPrice":
                listprice = currentprice.get('@priceValue')
            elif currentprice.get('@priceType') == "SalePrice":
...
productJSONData = json.dumps(products, indent=4, cls=ProductEncoder)
